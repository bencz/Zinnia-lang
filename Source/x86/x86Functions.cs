...
internal struct x86TestedLocation
{
...
internal struct x86TestedIdLocations
{
    ...
    public x86DataLocation[] GetSucceededLocations()
    {
        ...
        for (var i = 0; i < Locations.Length; i++)
            if (Locations[i].Succeeded)
                Ret.Add(Locations[i].Location);

        return Ret.ToArray();
    }
}

internal struct x86CheckedIdentifier
{
    public Identifier Identifier;
...
[Flags]
internal enum x86CheckerFlags : byte
{
    None = 0,
    NoBraches = 1,
    NoLoopRepetation = 2
...
internal struct x86CheckerState
{
...
internal sealed class x86OverwritingChecker : CodeContext
{
    ...
    private x86OverwritingChecker(x86CheckerFlags Flags, IdContainer Container, x86CheckerState CheckerState,
        x86TestedIdLocations TestedLocs)
        : base(Container)
    {
        CheckerFlags = Flags;
        Arch = State.Arch as x86Architecture;
        this.CheckerState = CheckerState;
        this.TestedLocs = TestedLocs;
    }

    public x86OverwritingChecker(x86CheckerFlags Flags, IdContainer Container, x86TestedIdLocations TestedLocs = new())
        : base(Container)
    {
    ...
    private static x86LocationExtraction ExtractLocation(ExpressionNode Node)
    {
        var Data = Node.Data.Get<x86NodeData>();
        return ExtractLocation(Data.Output);
    }

    private static x86LocationExtraction ExtractLocation(x86DataLocation Location)
    {
    ...
    private void InitIdentifiers(x86CheckedIdentifier[] Identifiers, x86TestedIdLocations TestedLocs)
    {
        var TestedIdIndex = -1;
        for (var i = 0; i < Identifiers.Length; i++)
        {
        ...
        CheckerState.Identifiers = Identifiers;
        this.TestedLocs = TestedLocs;
    }

    private void ConditionalAdd(x86FuncScopeData FSData, List<x86CheckedIdentifier> Identifiers, Identifier Id)
    {
    ...
    private int GetStructIndex(string Name)
    {
    ...
    private int GetStructIndex(Identifier Id)
    {
    ...
    private void OnIdentifierWritten(Identifier Id)
    {
        var IdData = Id.Data.Get<x86IdentifierData>();
    ...
    private bool HasSucceededLocation()
    {
        if (TestedLocs.Identifier == null)
            throw new InvalidOperationException();

        for (var i = 0; i < TestedLocs.Locations.Length; i++)
            if (TestedLocs.Locations[i].Succeeded)
                return true;

        return false;
    }

    private bool OnIdentifierRead(Identifier Id)
    {
        var Index = GetStructIndex(Id);
        if (Index != -1)
        {
    ...
    private void OnGRegLocationWritten(int Index, x86RegisterMask Mask)
    {
        OnLocationWritten(x =>
    ...
    private bool DisableLocation(x86DataList List)
    {
        return DisableLocation(x => !List.IsFree(x));
    }

    private bool DisableLocation(x86DataLocation Location)
    {
        if (Location == null) throw new ArgumentNullException("Location");
        return DisableLocation(x => x.Compare(Location, x86OverlappingMode.Partial));
    }

    private bool DisableGRegLocation(int Index, x86RegisterMask Mask)
    {
    ...
    private bool DisableLocation(Predicate<x86DataLocation> Func)
    {
        for (var i = 0; i < CheckerState.Identifiers.Length; i++)
            if (TestedLocs.Identifier != null)
            {
    ...
    private void OnLocationWritten(Predicate<x86DataLocation> Func)
    {
        for (var i = 0; i < CheckerState.Identifiers.Length; i++)
            if (TestedLocs.Identifier != null)
            {
    ...
    private void OnLocationWritten(x86DataLocation Location)
    {
        if (Location == null) throw new ArgumentNullException("Location");
        OnLocationWritten(x => x.Compare(Location, x86OverlappingMode.Partial));
    }

    private void OnLocationWritten(x86DataList List)
    {
        OnLocationWritten(x => !List.IsFree(x));
    }

    private void OnLocationWritten(x86TemporaryData TempData)
    {
    ...
    private bool CheckSaveChResults(ExpressionNode Node, x86NodeData Data)
    {
    ...
    public bool IsOverwritten(ExpressionNode Node, x86DataLocation MoveAndOpPos = null)
    {
        ...
        else if (Node is OpExpressionNode)
        {
            ...
            else if (Op == Operator.Call || Op == Operator.NewObject)
            {
                ...
                for (var i = 1; i < Ch.Length; i++)
                    if (RegList[i] == null && IsOverwritten(Ch[i]))
                        return true;
        ...
        if (!Calculated && Node.Children != null)
            for (var i = 0; i < Node.Children.Length; i++)
                if (IsOverwritten(Node.Children[i]))
                    return true;
    ...
    private bool CheckParameters(FunctionScope FS)
    {
        var FSData = FS.Data.Get<x86FuncScopeData>();
    ...
    private struct x86LocationExtraction
    {
        public readonly x86DataLocation Location;
...
public class x86VariableLocCalcer
{
    ...
    private void GetAllIdentifiers(IdContainer Container)
    {
    ...
    private int GetIdPriority(Identifier Id)
    {
        var IdData = Id.Data.Get<x86IdentifierData>();
    ...
    private void ConstructAllocationList(IdContainer Container)
    {
...
public static class x86Functions
{
    ...
    private static void MakeRefType(Variable Var)
    {
    ...
    private static void InitFunction(FunctionScope Scope)
    {
        var Arch = Scope.State.Arch as x86Architecture;
    ...
    private static void UpdateParamBaseRegisters(FunctionScope Scope)
    {
    ...
    private static void Reset(FunctionScope Scope)
    {
        var FSData = Scope.Data.Get<x86FuncScopeData>();
    ...
    private static void Reset(IdContainer Container)
    {
    ...
    private static int GetNeededSpaceForSaving(FunctionScope Scope, x86DataAllocator Allocator)
    {
        var Ret = 0;
    ...
    public static void CreateAssembly(FunctionScope FS)
    {
        ...
        {
            var Func = () => CG.LeaveFunction(FS, Allocator);
    ...
    private static void CalcExprRegs(List<ExpressionNode> Expressions, x86DataLocCalcer DataLocCalcer)
    {
        for (var i = 0; i < Expressions.Count; i++)
            DataLocCalcer.Calc(Expressions[i]);
    }

    private static bool CheckMem2Mem(List<ExpressionNode> Expressions, x86DataLocChecker DataLocChecker)
    {
        for (var i = 0; i < Expressions.Count; i++)
    ...
    private static void CalcStackPointers(FunctionScope FS)
    {
    ...
    private static List<x86MoveStruct> CalcLocations(FunctionScope FS)
    {
        var Arch = FS.State.Arch as x86Architecture;
        var FSData = FS.Data.Get<x86FuncScopeData>();
    ...
    private static x86DataLocation[] GetPossibleLocations(FunctionScope Scope,
    ...
    private static void ProcessParam(LocalVariable Id, List<x86MoveStruct> ToCopy)
    {
        var IdData = Id.Data.Get<x86IdentifierData>();
    ...
    private static void ProcessMemoryParam(LocalVariable Id, x86IdentifierData IdData)
    {
        ...
        if (IdData.ParamLocation.IsMemory() &&
    ...
    private static bool ProcessRegParam(LocalVariable Id, x86IdentifierData IdData)
    {
        var FS = Id.Container.FunctionScope;
        var FSData = FS.Data.Get<x86FuncScopeData>();
        var Allocator = FSData.Allocator;
        ...
        Func<x86DataLocation[], x86DataLocation> Selector = null)
    {
        ...
        if (Selector == null) IdData.Location = Locations[0];
        else IdData.Location = Selector(Locations);
        return IdData.Location != null;
        ;
    }

    private static void CalcParamDataLoc(FunctionScope Scope)
    {
        var Arch = Scope.State.Arch as x86Architecture;
        var Data = Scope.Data.Get<x86FuncScopeData>();
