...
public class x86DataList
{
    ...
    {
        if (Arrays.Memory != null) SetUsed(Arrays.Memory);
    ...
    {
        if (Pos is x86GRegLocation) return GRegisters.IsFree(Pos as x86GRegLocation);

        if (Pos is x86SSERegLocation) return !SSERegisters[(Pos as x86SSERegLocation).Index];

        if (Pos is x86StackLocation) return StackOffset <= (Pos as x86StackLocation).Offset;

        if (Pos is x86MultiLocation)
        {
            var MPos = Pos as x86MultiLocation;
            for (var i = 0; i < MPos.Locations.Length; i++)
...
public class x86DataAllocator : x86DataList
{
        ...
        : base(Container.State.Arch as x86Architecture, AllocRegsLists)
    {
        ...
        FuncScope = Container.FunctionScope;
        x86CallConv = Arch.GetCallingConvention(FuncScope.Type.CallConv);
        ContainerData = Container.Data.Get<x86IdContainerData>();
        FSData = FuncScope.Data.Get<x86FuncScopeData>();
    }

    private bool IsGRegisterFree(int Index, x86RegisterMask Mask, bool EnableParamLocs = false)
    {
    ...
    private bool IsSSERegisterFree(int Index, bool EnableParamLocs = false)
    {
    ...
    public x86DataLocation Allocate(Type T, x86DataLocationType DataCalcPos = x86DataLocationType.None,
    ...
    private x86GRegLocation AllocGRegHelper(int Size, bool OneByteVariant,
        x86GRegisterList CantBe = new(), bool EnableParamLocs = false)
    {
    ...
    public x86SSERegLocation AllocSSERegHelper(int Size, x86RegisterList CantBe = new(), bool EnableParamLocs = false)
    {
    ...
    public x86SSERegLocation AllocSSERegister(int Size, x86RegisterList CantBe = new())
    {
    ...
    public x86GRegLocation AllocGRegister(x86NeededGRegister Reg, x86GRegisterList CantBe = new())
    {
    ...
    private x86DataLocation AllocMultiPos(int Size, int Align, x86DataLocationType DataCalcPos, x86DataList CantBe,
        int PartSize)
    {
    ...
    public x86DataLocation GetAllocated(x86DataLocationType Type, int Size, int Align = -1)
    {
        if ((Type & x86DataLocationType.General) != 0)
        {
            ...
            if ((Type & x86DataLocationType.OneByte) == 0)
            {
                ...
                {
                    var Reg = Sequence[i];
                    if (Arch.IsGRegisterExists(Reg, Mask) && !Arch.IsGRegisterExists(Reg, 0, 1) &&
                        !GRegisters.IsFree(Reg, Mask))
                        return new x86GRegLocation(Arch, Reg, Mask);
                }
...
public struct x86RegisterList
{
    ...
    public bool Initialized => UsedRegs != null;

    public int Size => UsedRegs.Length;
...
public class x86DataSequence
{
    ...
    public int UsedCount => NextMaybeHighByte ? GRegisterIndex + 1 : GRegisterIndex;
    ...
    public bool CanAllocGReg(int Size = -1)
    {
        ...
        return GRegisterIndex + RegCount <= Options.GRegisters.Length;
    }
