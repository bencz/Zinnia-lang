...
public class x86Plugin : ExpressionPlugin
{
    ...
    public NCPlugin NCPlugin;
    public x86NodeFlags RootFlags;

    private readonly bool[] TupleAssignment_Link = { false, true };
        ...
        : base(Parent)
    {
        ...
        NCPlugin = Parent.GetPlugin<NCPlugin>();
    }
    ...
    private void SetTempCantBe(ExpressionNode Node, Action<x86DataList> Action)
    {
    ...
    private bool IsConstValue(ExpressionNode Node, BigInteger Value)
    {
    ...
    private bool AdjustRegs_SingleOp(ExpressionNode Node, x86NodeData Data)
    {
        var Ch = Node.Children;
        var Ch0Data = Ch[0].Data.Get<x86NodeData>();
        ...
        else if (Op == Operator.Cast)
        {
            ...
            else if (RFrom is NonFloatType)
            {
                ...
                if ((RFrom is UnsignedType && Size == Arch.RegSize) || Size > Arch.RegSize)
                {
                    ...
                    if (RTo.Size != 4) throw new ApplicationException();
    ...
    private bool ZeroTestingNode(OpExpressionNode OpNode)
    {
        var Ch = OpNode.Children;
        var Op = OpNode.Operator;
        var Type = Ch[0].Type.RealId as Type;
    ...
    public PluginResult NewOpNode_Pointer(ref ExpressionNode Node)
    {
        ...
        else if (Op == Operator.Index)
        {
            ...
            if (AddToCh1)
            {
                ...
                var NewCh = new[] { Ch[1], OpCh0.Children[1 - PointerIndex] };
    ...
    public PluginResult NewOpNode_RefArray(ref ExpressionNode Node)
    {
        ...
        //--------------------------------------------------------------------------------------
        if (Op != Operator.Assignment) return NewOpNode_Underlying(ref Node);
    ...
    public PluginResult NewOpNode_Structured(ref ExpressionNode Node)
    {
        ...
        if (Op == Operator.Negation || Op == Operator.Complement || Operators.IsBitArithmShift(Op))
        {
            ...
            {
                var Code = Node.Code;
                Node = Expressions.ExtractTupleOp(Node, Parent,
                    (i, Args) => { return Parent.NewNode(new OpExpressionNode(Op, Args, Code)); });
        ...
        else if (Op == Operator.Assignment)
        {
            ...
            if (Ch0Op == Operator.Tuple)
            {
                ...
                var Code = Node.Code;
                Node = Expressions.ExtractTupleOp(Node, Parent,
                    (i, Args) => { return Parent.NewNode(new OpExpressionNode(Operator.Assignment, Args, Code)); },
        ...
        else if (Op == Operator.Cast)
        {
            ...
            if (RTo is TupleType && RFrom is TupleType)
            {
                ...
                Node = Expressions.ExtractTupleOp(Node, Parent, (i, Nodes) =>
                {
                    ...
                    var NewCh = new[] { Nodes[0], NewTypeNode };
    ...
    public PluginResult NewOpNode_Char(ref ExpressionNode Node)
    {
        ...
        if (Operators.IsBoolRetBitArithmShift(Op))
        {
    ...
    public PluginResult NewOpNode_Type(ref ExpressionNode Node)
    {
        ...
        if (Type is FloatType) return NewOpNode_Float(ref Node);
        if (Type is EnumType) return NewOpNode_Enum(ref Node);
        if (Type is BooleanType) return NewOpNode_Bool(ref Node);
        if (Type is PointerType) return NewOpNode_Pointer(ref Node);
        if (Type is ReferenceType) return NewOpNode_Reference(ref Node);
        if (Type is StructuredType) return NewOpNode_Structured(ref Node);
        if (Type is TypeOfType) return NewOpNode_TypeOfType(ref Node);
        if (Type is TypeOfFunction) return NewOpNode_Function(ref Node);
        if (Type is NonrefArrayType) return NewOpNode_NonrefArray(ref Node);
        if (Type is CharType) return NewOpNode_Char(ref Node);
        if (Type is StringType) return NewOpNode_String(ref Node);
        if (Type is RefArrayType) return NewOpNode_RefArray(ref Node);
        if (Type is ObjectType) return NewOpNode_Structured(ref Node);
        throw new ApplicationException();
    }
    ...
    private static Type GetOpExpressionType(OpExpressionNode Node)
    {
        ...
        if (Ch != null && Ch.Length > 0) return Ch[0].Type.RealId as Type;
        return null;
    }
    ...
    private ExprRecResult NewCallOpNode_SpecialFunctions_SSE(ref ExpressionNode Node, x86NodeData Data)
    {
        var OpNode = Node as OpExpressionNode;
        var Ch = OpNode.Children;
        var Op = OpNode.Operator;
    ...
    private ExprRecResult NewCallOpNode_SpecialFunctions_FPU(ref ExpressionNode Node, x86NodeData Data)
    {
        var OpNode = Node as OpExpressionNode;
        var Ch = OpNode.Children;
        var Op = OpNode.Operator;
    ...
    public PluginResult NewCallOpNode(ref ExpressionNode Node)
    {
        ...
        if (Res != ExprRecResult.Unknown)
        {
            ...
            if (Res == ExprRecResult.Ready) return PluginResult.Ready;
            if (Res == ExprRecResult.Failed) return PluginResult.Failed;
            throw new ApplicationException();
        }
        ...
        //--------------------------------------------------------------------------------------
        var RetType = CalledFuncType.RetType;
    ...
    private LocalVariable FindFirstlyMovedId(ExpressionNode Node)
    {
        var Data = Node.Data.Get<x86NodeData>();
        if ((Data.Flags & x86NodeFlags.AllocateLocation) == 0)
            return null;
    ...
    public PluginResult NewOpNode_NonFloat_Cast(ref ExpressionNode Node)
    {
        ...
        if (RFrom is FloatType)
        {
            if (Arch.FloatingPointMode == x86FloatingPointMode.FPU)
            {
                ...
                if (RTo.Size == 1 || RTo is UnsignedType)
                {
                    var Global = Container.GlobalContainer;
    ...
    private void CalcFPUUsedRegs(x86NodeData Data, ExpressionNode Node)
    {
        if ((Data.Flags & x86NodeFlags.AllocateLocation) != 0)
        {
    ...
    private bool CalcNodeUsedData(x86NodeData Data, ExpressionNode Node)
    {
    ...
    private ExpressionNode CreateFPULoadNode(ExpressionNode Node)
    {
    ...
    private bool AdjustRegs_FPU(OpExpressionNode Node, bool Mod, bool RemoveIntCasts = true)
    {
        var Ch = Node.Children;
        ...
        if (Ch1Data.UsedFPUStack >= 8 && Mod && !x86Expressions.NeedLoadFloat(Ch[0], true))
        {
            ...
            if ((Ch[0] = CreateMoveNode(Ch[0], SpecifyDataCalcPos: true, DataCalcPos: x86DataLocationType.Memory)) ==
                null)
                return false;
    ...
    private bool AdjustRegs(OpExpressionNode Node, bool Mod)
    {
        var Ch = Node.Children;
