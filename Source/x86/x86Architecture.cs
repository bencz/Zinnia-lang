...
{
    private ClassType _x86HelperClass;
    public List<x86ConstString> ConstStrings = new();
...
public struct x86MoveStruct
{
    ...
    public x86MoveStruct Inverse => new(Src, Dst, TempData, Executor, StoredDataType);
}
...
public class x86Architecture : IArchitecture, INCArchitecture
{
    ...
    public int SSERegSize => (Extensions & x86Extensions.AVX) != 0 ? 32 : 16;
    public int RegCount => (Extensions & x86Extensions.LongMode) != 0 ? 16 : 8;
    public int ByteRegCount => (Extensions & x86Extensions.LongMode) != 0 ? 14 : 4;
    public x86RegisterMask RegisterMask => new(0, RegSize);
    public int RegSize => (Extensions & x86Extensions.LongMode) != 0 ? 8 : 4;
    public int MaxStructPow2Size => (Extensions & x86Extensions.SSE) != 0 ? SSERegSize : RegSize;
        ...
        for (var i = 0; i < Ch.Length; i++)
            if (Ch[i] is OpExpressionNode)
            {
                ...
                for (var j = 0; j < ChiCh.Length; j++)
                    if (ChiCh[j] is OpExpressionNode)
                        return false;
            }
    ...
    public bool ProcessFunction(Function Func)
    {
        ...
        return x86Functions.ProcessFunction(Scope);
    }
    ...
    public x86CallingConvention GetCallingConvention(CallingConvention Conv)
    {
        ...
        if (Conv == CallingConvention.StdCall) return CallingConventions.StdCall;
        if (Conv == CallingConvention.CDecl) return CallingConventions.CDecl;
        throw new ApplicationException();
    }
        ...
        OperatingSystem OperatingSystem, CallingConvention CallingConvention)
    {
        ...
        if (LongMode)
        {
            if (OperatingSystem == OperatingSystem.Windows)
            {
                ...
                {
                    GRegisters = new[] { 2, 1, 8, 9 },
                    SSERegisters = new[] { 0, 1, 2, 3 },
        ...
        else
        {
            ...
            var GRegAllocationSequence = new[]
            {
                0, 2, 1, 3, 5, 6, 7
            };
            ...
            if (CallingConvention == CallingConvention.ZinniaCall)
            {
                ...
                RetValue.AllocationSequence = new x86SequenceOptions
                {
                    ...
                    SSERegisters = new[] { 0, 1, 2, 3, 4, 5, 6, 7 },
                    AllowPartRegisters = true,
                    Align = 1
                };
                ...
                RetValue.ParameterSequence = new x86SequenceOptions
                {
                    GRegisters = new[] { 0, 2, 1 },
                    SSERegisters = new[] { 0, 1, 2 },
                    AllowPartRegisters = true,
                    Align = 4
                };
    ...
    private void InitializeCallingConventions()
    {
    ...
    private void Write(Stream Stream, Action<StreamWriter> Func)
    {
    ...
    private bool ProcessAssembly(Function Function)
    {
...
#warning WARNING
    ...
    private void ProcessRegisterParams_GReg(int Index, int Size, ProcessAsCallFunc Func, x86DataSequence DS)
    {
    ...
    public void ProcessRegisterParams(Type[] Params, CallingConvention CallConv, ProcessAsCallFunc Func,
        x86DataSequence DS = null)
    {
        if (DS == null)
        {
    ...
    private void ProcessMoveCondBranch(GlobalContainer Global, x86MoveCondBranch Branch)
    {
    ...
    private CondBranch GetJumpBranch(Command Comm)
    {
