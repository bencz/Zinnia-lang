...
public static class x86Identifiers
{
    ...
    public static string GetSSETypeString(x86StoredDataType StoredDataType)
    {
        ...
        if (Kind == x86TypeKind.Float)
        {
            ...
            if (Precision == 8) return "sd";
            throw new ApplicationException();
        }

        if (Kind == x86TypeKind.FloatVector)
        {
            if (Precision == 4) return "ps";
            if (Precision == 8) return "pd";
            throw new ApplicationException();
        }

        if (IsNonfloatTypeKind(Kind))
        {
            if (Precision == 1) return "b";
            if (Precision == 2) return "w";
            if (Precision == 4) return "d";
            if (Precision == 8) return "q";
            if (Precision == 16) return "dq";
            throw new ApplicationException();
        }
    ...
    public static x86TypeKind GetScalarTypeKind(Identifier Identifier)
    {
        if (!(Identifier.RealId is StructType))
        {
            ...
            if (Identifier.RealId is SignedType) return x86TypeKind.Signed;
    ...
    public static x86StoredDataType GetStoredDataType(Identifier Identifier)
    {
        ...
        if (Precision == -1 || Kind == x86TypeKind.Unknown)
            return new x86StoredDataType();
        return new x86StoredDataType(Kind, Precision);
    }
    ...
    public static x86DataLocationType GetPossibleLocations(Identifier Type)
    {
        ...
        if (RType is FloatType)
        {
            ...
            if (Arch.FloatingPointMode == x86FloatingPointMode.FPU) return x86DataLocationType.Memory;
            throw new NotImplementedException();
        }

        if (RType is StructType || RType is NonrefArrayType) return x86DataLocationType.Memory;
        if (RType.Size == 1) return x86DataLocationType.OneByteGeneral | x86DataLocationType.Memory;
        return x86DataLocationType.GRegMem;
    }
    ...
    public static x86DataLocationType GetPossibleLocations(Identifier Id, x86IdentifierData Data)
    {
        ...
        return GetPossibleLocations(Id.TypeOfSelf.RealId as Type);
    }
    ...
    {
        if (Type.RealId is NonrefArrayType) return true;
...
public static class x86Expressions
{
    ...
    public static x86DataProperties GetDataProperties(Identifier Type)
    {
        ...
        return new x86DataProperties(RType.Size, RType.Align,
            x86Identifiers.GetPossibleLocations(Type));
    }
    ...
    public static x86Operator Negate(x86Operator Op)
    {
        ...
        if (Op == x86Operator.BitTestNonZero) return x86Operator.BitTestZero;

        if (Op == x86Operator.IsNan) return x86Operator.IsNotNan;
        if (Op == x86Operator.IsNotNan) return x86Operator.IsNan;
        if (Op == x86Operator.IsInfinite) return x86Operator.IsFinite;
        if (Op == x86Operator.IsFinite) return x86Operator.IsInfinite;

        if (Op == x86Operator.IsCarryFlagSet) return x86Operator.IsCarryFlagZero;
        if (Op == x86Operator.IsCarryFlagZero) return x86Operator.IsCarryFlagSet;
        if (Op == x86Operator.IsParityFlagSet) return x86Operator.IsParityFlagZero;
        if (Op == x86Operator.IsParityFlagZero) return x86Operator.IsParityFlagSet;
        if (Op == x86Operator.IsZeroFlagSet) return x86Operator.IsZeroFlagZero;
        if (Op == x86Operator.IsZeroFlagZero) return x86Operator.IsZeroFlagSet;
        if (Op == x86Operator.IsSignFlagSet) return x86Operator.IsSignFlagZero;
        if (Op == x86Operator.IsSignFlagZero) return x86Operator.IsSignFlagSet;
        if (Op == x86Operator.IsOverflowFlagSet) return x86Operator.IsOverflowFlagZero;
        if (Op == x86Operator.IsOverflowFlagZero) return x86Operator.IsOverflowFlagSet;
        throw new ApplicationException();
    }
    ...
    public static bool NeedsInstructions(ExpressionNode Node)
    {
        ...
        if (Node is IdExpressionNode || Node is ConstExpressionNode || Node is LinkingNode) return false;
        ...
        if (Node is OpExpressionNode)
        {
            ...
            if (Op == Operator.Member) return NeedsInstructions(Ch[0]);
        }
    ...
    public static x86DataLocation GetLocation(x86Architecture Arch, ExpressionNode Node)
    {
        ...
        if (Node is OpExpressionNode)
        {
            ...
            if (Op == Operator.Assignment) return GetLocation(Arch, Ch[0]);
...
#warning WARNING, temp solution to allow ZinniaCore compilation
                ...
                if (IdCh1.Identifier is MemberFunction)
                {
        ...
        else if (Node is IdExpressionNode)
        {
            ...
            if ((Data.Flags & x86NodeFlags.IdentifierByRef) != 0)
            {
                ...
                var AddressTempGReg = Data.TempData.GetGRegister(x86TempGRegPurposeType.Index);
        ...
        else if (Node is DataPointerNode)
        {
            ...
            if (IdDescNode.DescPointerType == DataPointerType.IncBin)
    ...
    private static x86DataLocation GetDefaultIdLocation(x86Architecture Arch, Identifier Id)
    {
        Id = Id.RealId;
        if (Id is Type) return GetDescLocation(Arch, Id);

        if (Id is Function) return new x86NamedLabelPosition(Arch, Id.AssemblyName);
