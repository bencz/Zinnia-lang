...
public struct x86TemporaryData
{
    ...
    public x86DataLocation GetRegister(x86ExecutorType Executor, x86TempGRegPurposeType Purpose)
    {
        if (Executor == x86ExecutorType.SSE)
            return SSERegs[0];
        if (Executor == x86ExecutorType.General)
...
public struct x86NeededTempData
{
    ...
    private static int GetIndex(AutoAllocatedList<x86NeededGRegister> List, x86TempGRegPurpose Purpose)
    {
        for (var i = 0; i < List.Count; i++)
            if (List[i].Purpose.IsEquivalent(Purpose))
                return i;
    ...
    public bool NonZero => GRegisters.Count > 0 || SSERegisters > 0 || Memory > 0;
    ...
    public void MustHaveSSEOrGReg(x86Architecture Arch, x86TempGRegPurpose Purpose, int Size)
    {
        ...
        if (Size > Arch.RegSize && Size % 4 == 0 && (Arch.Extensions & x86Extensions.SSE2) != 0)
            MustHaveSSEReg();
        else MustHaveGReg(Arch, Purpose, Size);
    }
...
public static class x86DataLocCalcHelper
{
    public static bool CanUseSSEMove(x86DataLocation Dst, x86DataLocation Src)
    {
        ...
        if (!(Dst is x86MemoryLocation || Dst is x86SSERegLocation)) return false;
        if (!(Src is x86MemoryLocation || Src is x86SSERegLocation)) return false;
        if (Dst.Size <= Arch.RegSize || Dst.Size != Src.Size) return false;
        return true;
    }

    public static x86DataLocationType IntersectLocType(x86DataLocationType A, x86DataLocationType B,
        bool MemoryIfNull = false)
    {
        var Ret = A & B;
    ...
    public static bool IsMemory(this ExpressionNode Node, x86Architecture Arch,
        x86OverlappingMode Mode = x86OverlappingMode.Partial)
    {
        var Data = Node.Data.Get<x86NodeData>();
    ...
    private static bool CanUseDataLocation(ExpressionNode Node, x86DataLocation Location)
    {
        var RetValue = true;
        ForeachNodesWithSameOutput(Node, x =>
        {
    ...
    private static void SetIdentifierCantBe(ExpressionNode Node, x86IdentifierData IdData)
    {
    ...
    private static bool _CanUseAssignedLocation(ExpressionNode Node, Predicate<ExpressionNode> Func)
    {
        var Data = Node.Data.Get<x86NodeData>();
    ...
    private static bool _CanUseAssignVar(ExpressionNode Node, Identifier AssignVar)
    {
        ...
        if (AssignVar is LocalVariable)
        {
            ...
            SetIdentifierCantBe(Node, IdData);
        }

        return true;
    }

    private static bool _CanUseClassMemberAssign(ExpressionNode Node, ClassType Type, Identifier Member)
    {
    ...
    private static bool _CanUsePtrIndexAssign(ExpressionNode Node)
    {
        return _CanUseAssignedLocation(Node, x => Expressions.GetIdentifier(x) != null);
    }

    private static bool _CanUseNonrefIndexAssign(ExpressionNode Node, Identifier Indexed, ConstValue Index)
    {
        return _CanUseAssignedLocation(Node, x =>
        {
...
public class x86DataLocCalcer
{
    ...
    private void ForeachSetUsedNode(ExpressionNode Node, Action<ExpressionNode> Func)
    {
        var Op = Expressions.GetOperator(Node);
    ...
    private void SetUsed(ExpressionNode Node, x86TemporaryData TempData)
    {
    ...
    private void SetUsed(ExpressionNode Node, x86DataList UsedData)
    {
        ForeachSetUsedNode(Node, x =>
        {
            var xData = x.Data.Get<x86NodeData>();
    ...
    private void SetUsed(ExpressionNode Node, x86DataLocation Output)
    {
        ForeachSetUsedNode(Node, x =>
        {
            var xData = x.Data.Get<x86NodeData>();
...
{
    private x86UnknownMemory _UnknownMemory;
    public x86Architecture Arch;

    private bool Check_DontUseAssignVar;
    private int Check_RecheckIndex;
    public IdContainer Container;
    public ExpressionNode ExprNode;
    public x86NodeData RootData;
    public x86DataAllocator TempAllocator;
    ...
    private bool CheckNodes_CallAll(x86NodeData Data, Func<ExpressionNode, bool> Func)
    {
        var RetValue = true;
        for (var i = 0; i < Data.AllNodes.Count; i++)
            if (!Func(Data.AllNodes[i]))
                RetValue = false;

        return RetValue;
    }

    private x86UnknownMemory GetUnknownMemory(int Size)
    {
    ...
    private x86UnknownMemory GetUnknownMemory(Identifier Type)
    {
    ...
    private bool CheckVerify(ExpressionNode Node)
    {
        var Data = Node.Data.Get<x86NodeData>();
    ...
    private x86DataLocation GetLocationForNode(ExpressionNode Node)
    {
    ...
    private void CheckMain(ExpressionNode Node)
    {
        var Data = Node.Data.Get<x86NodeData>();
