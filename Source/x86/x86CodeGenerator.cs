...
public class x86CodeGenerator : CodeGenerator, INCCodeGenerator
{
    ...
    private static int GetSSECondition(Operator Op)
    {
        if (Op == Operator.Equality) return 0;
        if (Op == Operator.Inequality) return 4;
        if (Op == Operator.Less) return 1;
        if (Op == Operator.LessEqual) return 2;
        if (Op == Operator.Greater) return 6;
        if (Op == Operator.GreaterEqual) return 5;
    ...
    private void GetBranchCode(CondBranch Branch)
    {
    ...
    private void GetLabelsCode(ExpressionNode Condition, int ThenLabel, int ElseLabel,
        ...
        Operator Op = Operator.Unknown, x86Operator x86Op = x86Operator.Unknown)
    {
        ...
        if (Ch0Type is FloatType)
        {
            if (Arch.FloatingPointMode == x86FloatingPointMode.FPU)
            {
                ...
                if (x86Op == x86Operator.IsInfinite || x86Op == x86Operator.IsFinite)
                {
                ...
                if (x86Op == x86Operator.FloatZeroTesting)
                {
        ...
        if (Op == Operator.Unknown && x86Expressions.IsFlagOp(x86Op))
        {
    ...
    private void PushData(x86DataLocation Pos)
    {
        ProcessIndexMoves(Pos);
        var FuncScope = Container.FunctionScope;
    ...
    private void GetAddrOf(x86DataLocation Dst, x86DataLocation Pos, x86NodeData Data, bool Push = false)
    {
    ...
    private void ComputeLinkedNodes(ExpressionNode Node)
    {
        var Data = Node.Data.Get<x86NodeData>();
    ...
    private void MoveAndOp_BootRet(x86DataLocation Dst, OpExpressionNode OpNode, x86NodeData Data, bool x86Op = false)
    {
        var Op = OpNode.Operator;
        var Ch = OpNode.Children;
    ...
    private bool MoveAndOp(x86DataLocation Dst, ExpressionNode Node)
    {
        var Data = Node.Data.Get<x86NodeData>();
        var OpNode = Node as OpExpressionNode;
        ...
        else if (Op == Operator.Add || Op == Operator.Multiply || Operators.IsBitwise(Op))
        {
            ...
            if (ChSrc.Compare(Dst) &&
                (!(Dst is x86MemoryLocation) || (Data.DataCalcPos & x86DataLocationType.Memory) != 0))
            {
                ComputeLinkedNodes(Node);
                EmitExpression(Ch[0]);
                EmitExpression(Ch[1]);
    ...
    private CondBranch GetFPULoadBranch(ExpressionNode Node)
    {
        return new CodeCondBranch(CG =>
        {
            var x86CG = CG as x86CodeGenerator;
    ...
    private x86DataLocation GetNodePos_FPU(ExpressionNode Node, x86GetNodePosMode Mode = x86GetNodePosMode.Default,
        bool LoadIds = false)
    {
        var Data = Node.Data.Get<x86NodeData>();
        if (Node is ConstExpressionNode)
        ...
        if (Node is IdExpressionNode)
        {
            ...
            {
                if (Mode == x86GetNodePosMode.ReturnNull) return null;
                if (Mode == x86GetNodePosMode.PositionOnly) return Pos;
        ...
        else if (Op == Operator.Multiply)
        {
            if (SplSrc.Length == 1)
            {
                ...
                if (Data.UsedDataBySelf != null)
                {
                    ...
                    if (Dst.Size == 1)
                        EAXEDXUsed = UsedData.GRegisters[0].Size >= 2 && UsedData.GRegisters[0].Offset == 0;
                    else
                        EAXEDXUsed = UsedData.GRegisters[0].Size >= Dst.Size && UsedData.GRegisters[2].Size >= Dst.Size;
                }
    ...
    private void GetSSERegisterSaves(FunctionScope Scope, x86DataAllocator Allocator)
    {
    ...
    private int PushPopGRegs(x86DataAllocator Allocator, x86CallingConvention x86CallConv, bool Push)
    {
        ...
        if (Push)
        {
            for (var i = 0; i < UsedRegs.Size; i++)
                if (UsedRegs[i].Size > 0 && PushPopGReg(x86CallConv, Push, i))
                    Ret++;
        }
        else
        {
    ...
    private bool PushPopGReg(x86CallingConvention x86CallConv, bool Push, int Index)
    {
