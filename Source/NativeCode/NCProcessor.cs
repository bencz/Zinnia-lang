...
internal class NCFuncScopeData
{
...
public class NCProcessor
{
    ...
    private NCProcessor(FunctionScope FuncScope, INCArchitecture NCArch)
    {
    ...
    private bool ProcessRecursively(IdContainer Container)
    {
    ...
    private bool ProcessFunctionScope(FunctionScope FS)
    {
    ...
    private IdContainer ProcessCommand(Command Command, bool NoExtract = false)
    {
    ...
    private bool AddFreeScopes(CodeScopeNode Scope, int Count, CodeString Code)
    {
    ...
    private CodeScopeNode CreateScopeForCommand(Command Command)
    {
    ...
    private static void CopyIdentifiers(IdContainer Dst, IdContainer Src)
    {
    ...
    public IdContainer ExtractCommand(Command Command, NCExpressionRunBefores[] RunBefores)
    {
        ...
        if (Command.Type == CommandType.While || Command.Type == CommandType.DoWhile)
        {
    ...
    private bool CallInitializer(CodeScopeNode Scope, ref int Index, PluginRoot Plugin,
    ...
    private bool CallInitializerWithCmp(CodeScopeNode Scope, ref int Index, PluginRoot Plugin, ExpressionNode Size,
        CodeString Code)
    {
    ...
    private bool CallInitializerWithoutCmp(CodeScopeNode Scope, ref int Index, PluginRoot Plugin, ExpressionNode Size,
        CodeString Code)
    {
    ...
    private bool AllocateObject(CodeScopeNode Scope, ref int Index, SelfVariable Self, ExpressionNode Size,
        PluginRoot Plugin, CodeString Code)
    {
        var TypeMngrPlugin = Plugin.GetPlugin<TypeMngrPlugin>();
    ...
    private static bool CreateFinallyReturn(FunctionScope Scope)
    {
        var Data = Scope.Data.Get<NCFuncScopeData>();
        var State = Scope.State;
        var RetType = Scope.Type.RetType;

        var Name = new CodeString("_" + State.AutoLabel);
    ...
    private bool SetFinallyReturn(CodeScopeNode Scope, ExpressionNode Value, CodeString Code)
    {
        var FS = Scope.FunctionScope;
        var FSData = FS.Data.Get<NCFuncScopeData>();
    ...
    private bool FinallyRethrow(Command Command, CodeScopeNode Scope, CodeString Code)
    {
        var CatchVar = GetCatchVariable(Command);
        ...
        var Cmp_Ch = new[] { Cmp_Dst, Cmp_Src };
    ...
    private static Variable GetCatchVariable(Command Command)
    {
        var NCData = Command.Data.GetOrCreate<NCCommandData>();
    ...
    private bool EnterTryBlock(CodeScopeNode Scope, int Index, CodeString Code, Identifier CatchVariable, int Label)
    {
        ...
        var Ch = new[] { CatchVarNode };
        CatchVarNode = Plugin.NewNode(new OpExpressionNode(Operator.Address, Ch, Code));
        var JumpTo = Plugin.NewNode(new LabelExpressionNode(Code, "_" + Label));
    ...
    private bool LeaveTryBlock(CodeScopeNode Scope, CodeString Code)
    {
        var Global = Scope.State.GlobalContainer;
        var Plugin = Scope.GetPlugin();
        if (!Plugin.Begin()) return false;
    ...
    private bool FinallyJump(CodeScopeNode Scope, Identifier JumpTo, CodeString Code)
    {
        var Plugin = Scope.GetPlugin();
        if (!Plugin.Begin()) return false;
        ...
        var Cmp_Ch = new[] { Cmp_IdNode, Cmp_Null };
        var Cmp_Node = Plugin.NewNode(new OpExpressionNode(Operator.Inequality, Cmp_Ch, Code));
    ...
    private static bool CreateFinallyJump(Command TryComm, CodeString Code)
    {
    ...
    private bool InitializeFinallyJump(Command TryComm, CodeString Code)
    {
    ...
    private bool SetFinallyJump(Command TryComm, CodeScopeNode Scope, int Label, CodeString Code)
    {
        Scope.FunctionScope.NeverSkippedLabels.Add(Label);
        return SetFinallyJump(TryComm, Scope, "_" + Label, Code);
    }

    private bool SetFinallyJump(Command TryComm, CodeScopeNode Scope, string Label, CodeString Code)
    {
        var NCData = TryComm.Data.GetOrCreate<NCCommandData>();
    ...
    private bool CreateJump(CodeScopeNode Scope, int Label, CodeString Code)
    {
        Scope.FunctionScope.NeverSkippedLabels.Add(Label);
        return CreateJump(Scope, "_" + Label, Code);
    }

    private bool CreateJump(CodeScopeNode Scope, string Label, CodeString Code)
    {
    ...
    private static Command CreateJump(IdContainer Container, string Label, CodeString Code)
    {
    ...
    private static Command CreateJump(IdContainer Container, ExpressionNode To, CodeString Code)
    {
