...
public class MTCompiler
{
    ...
    public bool Run()
    {
        ...
        for (var i = 0; i < Identifiers.Count; i++)
            if (!Identifiers[i].CalculateLayout())
                Result = false;
...
public class GlobalContainer : IdContainer
{
    ...
    public List<GlobalVariable> ExprConsts = new();
    internal Dictionary<string, Identifier> FastIds = new();
    ...
    public List<IdentifierAlias> TypeAliases = new();
    ...
    public override IdentifierAccess DefaultAccess => IdentifierAccess.Internal;

    public string AssemblyDescLabel => OutputAssembly.DescLabel;

    public string GlobalPointersLabel => "_%" + OutputAssembly.DescName + "_GlobalPointers";
...
#warning WARNING
        if ((Flags & GlobalContainerFlags.HasUninitedValues) != 0 || true)
        {
            ...
            var ZeroMemSizeCastCh = new[] { ZeroMemSizeCh0, ZeroMemSizeTypeNode };
            var ZeroMemSize = Plugin.NewNode(new OpExpressionNode(Operator.Cast, ZeroMemSizeCastCh, Name));
            if (ZeroMemSize == null) return false;

            var ZeroMemCh = new[] { ZeroMemFuncNode, ZeroMemAddress, ZeroMemSize };
    ...
    private static bool CallStaticConstructors(IdContainer Container, CodeScopeNode Scope, PluginRoot Plugin,
        CodeString Code)
    {
    ...
    public IncludedBinary GetIncludedBinary(string Name)
    {
        for (var i = 0; i < IncludedBinaries.Count; i++)
            if (IncludedBinaries[i].Name == Name)
                return IncludedBinaries[i];
    ...
    private static bool ProcessClassBases(IdContainer Container)
    {
        var RetValue = true;
    ...
    private static bool ProcessScopes(IdContainer Container, List<Identifier> Out)
    {
        var RetValue = true;
        if (Container.IsNotLoadedAssemblyScope())
        {
    ...
    private static bool ProcessStructureIdentifiers(IdContainer Container)
    {
        var RetValue = true;
        if (Container.IsNotLoadedAssemblyScope())
        {
    ...
    private bool ProcessTypes(List<Identifier> Out)
    {
    ...
    public bool SearchCommonIdentifiers()
    {
        ...
        CommonIds.UInt16.UnderlyingType =
            Identifiers.GetMember(State, System, "UInt16", StructOptions) as StructuredType;
        CommonIds.UInt32.UnderlyingType =
            Identifiers.GetMember(State, System, "UInt32", StructOptions) as StructuredType;
        CommonIds.UInt64.UnderlyingType =
            Identifiers.GetMember(State, System, "UInt64", StructOptions) as StructuredType;

        CommonIds.Single.UnderlyingType =
            Identifiers.GetMember(State, System, "Single", StructOptions) as StructuredType;
        CommonIds.Double.UnderlyingType =
            Identifiers.GetMember(State, System, "Double", StructOptions) as StructuredType;

        CommonIds.Boolean.UnderlyingType =
            Identifiers.GetMember(State, System, "Boolean", StructOptions) as StructuredType;
        CommonIds.Object.UnderlyingType =
            Identifiers.GetMember(State, System, "Object", ClassOptions) as StructuredType;
        CommonIds.String.UnderlyingType =
            Identifiers.GetMember(State, System, "String", ClassOptions) as StructuredType;
    ...
    private void Initialize()
    {
        ...
        // -----------------------------------------------------------------------------------
        var ConstRec = State.Language.Root.GetObject<NumberRecognizer>();
        ConstRec.TypeCodes = new[]
        {
            new("sb", CommonIds.SByte),
            new NumberTypeCodeDefinition("b", CommonIds.Byte),
            new NumberTypeCodeDefinition("s", CommonIds.Int16),
            new NumberTypeCodeDefinition("us", CommonIds.UInt16),
            new NumberTypeCodeDefinition("i", CommonIds.Int32),
            new NumberTypeCodeDefinition("u", CommonIds.UInt32),
            new NumberTypeCodeDefinition("l", CommonIds.Int64),
            new NumberTypeCodeDefinition("ul", CommonIds.UInt64),
            new NumberTypeCodeDefinition("p", CommonIds.IntPtr),
            new NumberTypeCodeDefinition("up", CommonIds.UIntPtr),
            new NumberTypeCodeDefinition("f", CommonIds.Single),
            new NumberTypeCodeDefinition("d", CommonIds.Double)
        };
    ...
    private void GetExprConstsCode(CodeGenerator CG)
    {
        var Arch = State.Arch;
    ...
    private bool HasReferencableAssemblyName(Identifier Id)
    {
        if ((Id.Flags & IdentifierFlags.Abstract) != 0) return false;
        return Id is Function || Id is GlobalVariable;
    }

    private void GetExternPublicCode(StringBuilder Out, List<string> AlreadyExported,
        AutoAllocatedList<Identifier> List)
    {
