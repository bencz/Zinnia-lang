...
public static class Commands
{
    ...
    public static bool CreateCatchVariable(Command TryComm,
        Identifier Type = null, CodeString Name = new())
    {
    ...
    private static void CreateRootCatchScope(Command TryComm)
    {
        TryComm.CatchScope = new CodeScopeNode(TryComm, TryComm.Code);
        TryComm.Children.Add(TryComm.CatchScope);
    }

    private static bool CopyCatchVar(CodeScopeNode Scope, Identifier From, Identifier To, CodeString Code)
    {
    ...
    private static bool CopyCatchVar(Command TryComm, CodeScopeNode Scope, Identifier Type, CodeString Name,
        CodeString Code)
    {
    ...
    private static ExpressionNode CreateCatchCondition(Command TryComm, Command Condition, Identifier Type,
        CodeString Code)
    {
        var CatchVar = GetOrCreateCatchVariable(TryComm);
...
{
    private ConditionResult _WillRun;
    private bool _WillRun_Calced;
    ...
    public bool HasExpressions => Expressions != null && Expressions.Count > 0;
        ...
        where T : IdContainer
    {
        ...
        {
            ForEachParent(Func, FunctionScope.Parent);
        }
        else if (Type == CommandType.Break || Type == CommandType.Continue)
        {
            var NeededFlag = Type == CommandType.Break ? CommandFlags.Breakable : CommandFlags.Continueable;

            var Loop = GetParent<Command>(x => (x.Flags & NeededFlag) != 0);
            ForEachParent(Func, Loop.Children[0]);
        }
        else if (Type == CommandType.Goto)
        {
            ForEachParent(Func, JumpTo.Parent);
        }
        else
        {
    ...
    public T JumpsOver<T>(Predicate<T> Func)
        where T : IdContainer
    {
        if (Type == CommandType.Return) return GetParent(Func, FunctionScope.Parent);
        ...
        if (Type == CommandType.Break || Type == CommandType.Continue)
        {
            ...
            var Loop = GetParent<Command>(x => (x.Flags & NeededFlag) != 0);
            return GetParent(Func, Loop.Children[0]);
        }

        if (Type == CommandType.Goto)
            return GetParent(Func, JumpTo.Parent);
        throw new InvalidOperationException();
    }
    ...
    private Command GetCommand(IdContainer Container)
    {
        var Scope = Container as CodeScopeNode;
        if (Scope == null) return null;

        if (Scope.Children.Count != 1) return null;
        return Scope.Children[0] as Command;
    }

    private void CalcIfAssembly(CodeGenerator CG, int Pos)
    {
        for (var i = Pos; i < Children.Count; i++)
        {
            ...
            if (Res == ConditionResult.Unknown)
            {
                ...
                CG.EmitCondition(Cond, Branches[0], Branches[1], NextLabel);
                break;
            }
