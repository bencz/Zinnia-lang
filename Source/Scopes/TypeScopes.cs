...
public abstract class TypeScope : IdentifierScope
{
    ...
    public override IdentifierAccess DefaultAccess => IdentifierAccess.Private;
}
...
public class EnumScope : TypeScope
{
    ...
    public override Identifier Identifier => EnumType;
...
public class StructuredScope : TypeScope
{
    ...
    public override Identifier Identifier => StructuredType;

    private bool NeedsConstructor
    {
        get
        {
    ...
    public bool CheckCycleInStructs()
    {
        if (StructuredType is StructType)
            for (var i = 0; i < IdentifierList.Count; i++)
                if (!CheckCycleInStructs(IdentifierList[i]))
                    return false;
    ...
    private bool ProcessId(Identifier Id)
    {
        if ((Id.Flags & IdentifierFlags.Override) != 0)
        {
            ...
            if ((OverriddenId.Flags & IdentifierFlags.Virtual) == 0)
            {
                State.Messages.Add(MessageId.OverrideNonvirtual, Id.Declaration);
                return false;
            }

            if ((OverriddenId.Flags & IdentifierFlags.Sealed) != 0)
            {
    ...
    public Constructor CreateDeclaredConstructor(CodeString Declaration, FunctionParameter[] Params,
    ...
    public bool ProcessBase()
    {
        ...
        for (var i = 0; i < Type.BaseStructures.Length; i++)
        {
            ...
            if ((RealBase.Flags & IdentifierFlags.Static) != 0)
            {
                State.Messages.Add(MessageId.CannotInheritStatic, BaseData.Name);
                return false;
            }

            if ((RealBase.Flags & IdentifierFlags.Sealed) != 0)
            {
