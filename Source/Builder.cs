...
public class ZinniaBuilder
{
    ...
    private readonly string _archiver;

    private readonly string _archivesDir;
    private string _asmFile;
    private readonly string _assembler;
    ...
    private bool _executeApp;
    private List<IncBinReference> _incBins;
    private Language _language;
    private readonly string _librariesDir;
    private readonly string _linker;

    private readonly Dictionary<string, string> _macroes;
    ...
    {
        foreach (var e in _macroes.Where(e => str.Contains(e.Key))) str = str.Replace(e.Key, e.Value);
    ...
    private static string GetString(string[] args, int i)
    {
        if (i >= args.Length)
        {
    ...
    {
        return files.Select(e => new CodeFile(e, File.ReadAllText(e), tabSize)).ToArray();
    }
