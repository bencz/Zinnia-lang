...
public static class ZinniaHelper
{
    ...
    public static int GetNextLinePosition(CodeFile File, int Line, int Indent = -1)
    {
        ...
        return File.GetLinePosition(Next);
    }
...
{
    public static string[] ExplicitImplicit = { "explicit", "implicit" };
    ...
    public bool ProcDeclarationLine(NonCodeScope Scope, CodeString Code)
    {
        ...
        var LeftSide = new CodeString();
        var OpFindRes = -1;

        if (FuncLine.Length > 0 && FuncLine[FuncLine.Length - 1] == ')')
        {
            BracketStart = FuncLine.GetBracketPos(State, true);
        ...
        if (BracketStart != -1)
        {
            ...
            if (OpFindRes != -1)
            {
                ...
                var Res = LeftSide.StartsWith(ExplicitImplicit);
                if (Res.Index != -1)
                {
                ...
                else
                {
                    ...
                    Name = new CodeString("%Operator_" + Op);
...
{
    public static string[] Strings = { "class", "struct", "enum", "flag" };
    ...
    private StructureBase[] GetBaseList(CompilerState State, CodeString Code)
    {
...
{
    public override string SelfName => "this";

    public override string BaseName => "base";
...
{
    public static string[] Strings = { "''", "rem" };
...
{
    public static string[] Strings = { "namespace", "using" };
...
public class ZinniaCommandInnerSeparator : LanguageNode, ICommandInnerSeparator
{
        ...
        CommandInnerSeparatorFlags Flags = CommandInnerSeparatorFlags.None)
    {
        ...
        if ((Flags & CommandInnerSeparatorFlags.InnerIsOptional) == 0)
        {
