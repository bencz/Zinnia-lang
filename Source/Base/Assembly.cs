...
public class Assembly
{
    ...
    public string DescLabel => "_%" + DescName;

    private void CalculateDescName()
    {
...
public class AssemblyDescLoader
{
    ...
    private Assembly[] ChildAssemblies;

    private Assembly CurrentAssembly;
    private GlobalContainer Global;
    private LoaderIdentifier[] LoaderIds;
    private BinaryReader Reader;

    private CompilerState State;
    private AutoAllocatedList<Identifier> UpdateList;
    ...
    private void Dereference(Identifier Id, List<LoaderReference> List)
    {
        if (List == null) return;
    ...
    private void SetReferencedId(LoaderReference Ref, Identifier Id)
    {
    ...
    private void ReadReference(Identifier DstId, ReferenceDestination Dest, int Index)
    {
        ReadReference(new LoaderReference(DstId, Dest, Index));
    }

    private void ReadReference(LoaderReference Ref)
    {
    ...
    private void ReadParameterReferences(Identifier Id)
    {
    ...
    private void ReadParameterReferences(Identifier Id, int Start, int ParamCount)
    {
    ...
    private FunctionParameter ReadParameterReference(Identifier Id, bool HasDefaultValue)
    {
    ...
    private void ReadReferenceDeclared(Identifier DstId, ReferenceDestination Dest, int Index)
    {
        ReadReferenceDeclared(new LoaderReference(DstId, Dest, Index));
    }

    private void ReadReferenceDeclared(LoaderReference Ref)
    {
    ...
    private bool ReadChildAssemblies(GlobalContainer Global)
    {
        var RetValue = true;
        var Count = ReadLEB128_Int();
    ...
    private ConstValue ReadConst()
    {
        ...
        if (T == ConstValueType.Double) return new DoubleValue(Reader.ReadDouble());
        if (T == ConstValueType.Float) return new FloatValue(Reader.ReadSingle());
        if (T == ConstValueType.Boolean) return new BooleanValue(Reader.ReadBoolean());
        if (T == ConstValueType.Char) return new CharValue(Reader.ReadChar());
        if (T == ConstValueType.String) return new StringValue(ReadLEB128_String());
        if (T == ConstValueType.Zero) return new ZeroValue();
        if (T == ConstValueType.Null) return new NullValue();
        throw new InvalidAssemblyException("Invalid constant type");
    }

    private Identifier ReadIdentifer(IdContainer Container, Identifier Parent)
    {
...
#warning CHECK
        ...
        {
            EnumType Enum;
            if (DeclaredIdType == DeclaredIdType.Enum)
        ...
        else if (DeclaredIdType == DeclaredIdType.Function || DeclaredIdType == DeclaredIdType.Constructor)
        {
            ...
            if (DeclaredIdType == DeclaredIdType.Function)
            {
    ...
    private void ReadIdList(IdContainer Container, Identifier Parent)
    {
        var Count = ReadLEB128_Int();
        if (Count <= 0)
        {
    ...
    private enum ReferenceDestination
    {
        Children,
        RealId,
        OverriddenId,
    ...
    private struct LoaderReference
    {
        public readonly Identifier DstId;
        public readonly ReferenceDestination Dest;
    ...
    private class LoaderIdentifier
    {
...
public class AssemblyDescCreator
{
    ...
    private void ReferenceParameters(Identifier[] Ch, int Start, int Count, int RequiredParams)
    {
    ...
    private void ReferenceParameter(FunctionParameter Param)
    {
    ...
    private void WriteLEB128(BigInteger Value)
    {
    ...
    private void WriteLEB128(uint Value)
    {
        LEB128Helper.Encode(Value, Writer.Write);
    }
    ...
    private void WriteLEB128(ulong Value)
    {
        LEB128Helper.Encode(Value, Writer.Write);
    }

    private void WriteConst(ConstValue Value)
    {
        var T = Value.Type;
        Writer.Write((byte)T);
    ...
    private ushort GetFlagData(Identifier Id)
    {
    ...
    private void WriteIdentifier(IEnumerable<IdContainer> List, Identifier Parent)
    {
        var Count = 0;
    ...
    private void WriteIdentifier(AutoAllocatedList<Identifier> List, Identifier Parent)
    {
        if (List.List == null)
        {
            WriteLEB128(0);
    ...
    private void WriteIdentifier(List<Identifier> List, Identifier Parent)
    {
    ...
    private void WriteIdentifier(Identifier Id, Identifier Parent)
    {
