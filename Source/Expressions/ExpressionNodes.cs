...
public static class Operators
{
    ...
    {
        var Op = GetOperator(OpStr.ToString(), Params);
    ...
    {
        return IsBoolRetBitArithmShift(Op) || Op == Operator.Negation ||
               Op == Operator.UnaryPlus || Op == Operator.Not || Op == Operator.Complement ||
...
public class DataPointerNode : ExpressionNode
{
    ...
    public DataPointerNode(CodeString Code, Assembly Assembly, ExpressionFlags Flags = ExpressionFlags.None)
        : base(Code, Flags)
    {
        DescPointerType = DataPointerType.Assembly;
    ...
    public DataPointerNode(CodeString Code, Identifier Id, ExpressionFlags Flags = ExpressionFlags.None)
        : base(Code, Flags)
    {
        DescPointerType = DataPointerType.Identifier;
        this.Id = Id;
    }
    ...
    public DataPointerNode(CodeString Code, IncludedBinary IncBin, ExpressionFlags Flags = ExpressionFlags.None)
        : base(Code, Flags)
    {
        DescPointerType = DataPointerType.IncBin;
        this.IncBin = IncBin;
    }
...
public class NamedParameterNode : ExpressionNode
{
    ...
    public NamedParameterNode(CodeString Code, ExpressionNode Child, CodeString Name,
...
public class OpExpressionNode : ExpressionNode
{
    ...
    public OpExpressionNode(Operator Operator, ExpressionNode[] Children, CodeString Code,
        ExpressionFlags Flags = ExpressionFlags.None)
        : base(Code, Flags)
    {
        this.Operator = Operator;
...
public class LinkingNode : ExpressionNode
{
    ...
    public override ExpressionNode RealNode => LinkedNode.Node;
...
public abstract class ExpressionNode
{
    ...
    public virtual ExpressionNode RealNode => this;
    ...
    public virtual ConditionResult ConditionResult => ConditionResult.Unknown;
    ...
    private static PluginResult CallNewNode_NoBeginEnd(ref ExpressionNode Node, PluginRoot Plugin)
    {
    ...
    public bool CheckNodes(Predicate<ExpressionNode> Func, bool CheckRoot = true)
    {
        ...
        for (var i = 0; i < LinkedNodes.Count; i++)
            if (!LinkedNodes[i].Node.CheckNodes(Func))
                return false;

        if (Children != null)
            for (var i = 0; i < Children.Length; i++)
                if (!Children[i].CheckNodes(Func))
                    return false;
    ...
    public ExpressionNode Copy(PluginRoot Plugin, PluginFunc Func = null,
        BeginEndMode Mode = BeginEndMode.Both, CodeString Code = new(), bool LeftType = false)
    {
        if ((Mode & BeginEndMode.Begin) != 0 && !Plugin.Begin())
            return null;
