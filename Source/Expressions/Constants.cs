...
public static class Constants
{
    ...
    public static bool RecognizeString(CodeString Code, PluginRoot Plugin, out string Out)
    {
        ...
        if (!Recognize(Code, StringType, Plugin, out Value))
            return false;
    ...
    public static ConstExpressionNode GetIntValue(IdContainer Container, int Val, CodeString Code,
...
public abstract class ConstValue
{
    ...
    public virtual double Double => throw new InvalidOperationException();
    ...
    private byte[] AdjustBytes(int Size, bool Signed)
    {
    ...
    public virtual unsafe long GetSigned(int Offset, int Size)
    {
        ...
        fixed (byte* pBytes = Bytes)
        {
            ...
            if (Size == 1) return *(sbyte*)pBytesOffset;
            if (Size == 2) return *(short*)pBytesOffset;
            if (Size == 4) return *(int*)pBytesOffset;
            if (Size == 8) return *(long*)pBytesOffset;
            throw new ApplicationException();
        }
    }
    ...
    public virtual unsafe ulong GetUnsigned(int Offset, int Size)
    {
        ...
        {
            var pBytesOffset = pBytes + Offset;
            if (Size == 1) return *pBytesOffset;
            if (Size == 2) return *(ushort*)pBytesOffset;
            if (Size == 4) return *(uint*)pBytesOffset;
            if (Size == 8) return *(ulong*)pBytesOffset;
            throw new ApplicationException();
        }
    }
}
...
public class StructuredValue : ConstValue
{
    ...
    {
        Members = new List<ConstValue>();
    }

    public override ConstValueType Type => ConstValueType.Structure;
    ...
    public StructuredValue Convert(ConstValueType[] To)
    {
        ...
        return new StructuredValue(Ret);
    }
    ...
    private StructuredValue ForAllMembers(Func<ConstValue, ConstValue> Func)
    {
    ...
    private StructuredValue ForAllMembers(StructuredValue Src, Func<ConstValue, ConstValue, ConstValue> Func)
    {
        var NewMembers = new List<ConstValue>();
        for (var i = 0; i < Members.Count; i++)
        {
    ...
    private StructuredValue ForAllMembers(Identifier Type, Func<ConstValue, Identifier, ConstValue> Func)
    {
    ...
    private StructuredValue ForAllMembers(StructuredValue Src, Identifier Type,
        Func<ConstValue, ConstValue, Identifier, ConstValue> Func)
    {
        var SType = Type.RealId as StructuredType;
        var SMembers = SType.StructuredScope.IdentifierList;
        if (SMembers.Count != Members.Count)
...
public class IntegerValue : ConstValue
{
    ...
    public override ConstValueType Type => ConstValueType.Integer;
    ...
    public override ConstValue DoOperation(ConstValue Src, Operator Op, Identifier Type)
    {
        if (Op == Operator.Complement) return new IntegerValue(DataStoring.WrapToType(~Value, Type));
    ...
    public override bool CheckBounds(Identifier Type)
    {
        ...
        if (Type.RealId is PointerType || Type.RealId is ClassType) return true;

        if (Type.RealId is EnumType)
        {
            var EType = Type.RealId as EnumType;
            if (EType.TypeOfValues == null) return true;
            return CheckBounds(EType.TypeOfValues);
        }

        if (Type.RealId is ObjectType) return false;
...
public class DoubleValue : ConstValue
{
    ...
    public override double Double => Value;

    public override ConstValueType Type => ConstValueType.Double;

    public override unsafe byte[] ToByteArray()
    {
...
public class FloatValue : ConstValue
{
    ...
    public override ConstValueType Type => ConstValueType.Float;
...
public class StringValue : ConstValue
{
    ...
    public override ConstValueType Type => ConstValueType.String;
...
public class CharValue : ConstValue
{
    ...
    public override ConstValueType Type => ConstValueType.Char;
    ...
    public override ConstValue Convert(ConstValueType To)
    {
        if (To == ConstValueType.Integer) return new IntegerValue(new BigInteger(Value));
...
public class BooleanValue : ConstValue
{
    ...
    public override ConstValueType Type => ConstValueType.Boolean;
...
{
    public override ConstValueType Type => ConstValueType.Zero;
    ...
    public override long GetSigned(int Offset, int Size)
    {
        return 0;
    }

    public override ulong GetUnsigned(int Offset, int Size)
    {
        return 0;
    }
...
{
    public override ConstValueType Type => ConstValueType.Null;
...
public class ConstExpressionNode : ExpressionNode
{
    ...
    public ConstExpressionNode(Identifier Type, ConstValue Value, CodeString Code,
    ...
    public override ConditionResult ConditionResult => Bool ? ConditionResult.True : ConditionResult.False;
    ...
    protected override ExpressionNode Copy_WithoutLinkedNodes(CompilerState State, CodeString Code,
        Func<ExpressionNode, ExpressionNode> Func)
    {
