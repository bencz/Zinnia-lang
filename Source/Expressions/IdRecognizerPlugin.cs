...
internal struct ScopeResolutions
{
...
public class IdRecognizerPlugin : ExpressionPlugin
{
    ...
    private ExpressionNode GetId(CodeString Code, bool EnableMessages,
        OverloadSelectionData OverloadData = new(), IdContainer Container = null)
    {
        if (Container == null)
    ...
    private ExpressionNode DeclareVar(CodeString Name, Identifier T)
    {
        if (OnDeclaration != null) return OnDeclaration(Parent, Name, T, Name);
        return Parent.DeclareVarAndCreateIdNode(Name, T, Name);
    }

    private ExpressionNode GetOrDeclareId(CodeString Name, bool EnableMessages, Identifier T = null,
        OverloadSelectionData OverloadData = new())
    {
    ...
    private ExpressionNode ResolveNode(ExpressionNode Node, Identifier DeclarationType = null,
        OverloadSelectionData OverloadData = new(), ResolveNodeFlags Flags = ResolveNodeFlags.None)
    {
    ...
    {
        return ForceContinue(ref Node);
    }

    private Type GetType(ExpressionNode Node, out bool Static)
    {
        var Type = (Type)null;
        if (Node.Type != null)
    ...
    private PluginResult ForceContinue(ref ExpressionNode Node, Identifier DeclarationType = null,
        OverloadSelectionData OverloadData = new(), ResolveNodeFlags Flags = ResolveNodeFlags.None)
    {
        if (Node is StrExpressionNode)
        {
    ...
    private PluginResult ResolveNode(ref ExpressionNode Node, Identifier DeclarationType = null,
        OverloadSelectionData OverloadData = new(), ResolveNodeFlags Flags = ResolveNodeFlags.None)
    {
    ...
    private PluginResult ForceContinue_ScopeResolution(ref ExpressionNode Node,
        OverloadSelectionData OverloadData = new())
    {
        var Ch = Node.Children;
    ...
    private PluginResult ForceContinue_MemberNode(ref ExpressionNode Node,
        OverloadSelectionData OverloadData = new())
    {
        var Ch = Node.Children;
    ...
    private bool ResolveParams(ExpressionNode[] Ch)
    {
    ...
    private PluginResult ProcessParams(ref ExpressionNode Node)
    {
        var Ch = Node.Children;
        var Type = Ch[0].Type;
        if (Type == null)
            ...
            if ((State.Language.Flags & LangaugeFlags.ConvertParametersToTuple) != 0)
                if (FuncType.Children.Length == 2 && Ch.TrueForAll(x => !(x is NamedParameterNode)))
                {
                    ...
                    if (Param0.TypeOfSelf.RealId is TupleType)
                    {
                        ...
                        Node.Children = Ch = new[] { Ch[0], NewCh1 };
                        return PluginResult.Succeeded;
                    }
                }
        ...
        for (var i = 1; i < Ret.Length; i++)
            if (Ret[i] == null)
            {
                ...
                if (Param.ConstInitValue != null)
                {
                    ...
                    if ((Ret[i] = Parent.NewNode(Ret[i])) == null) RetValue = false;
                }
                else
                {
    ...
    private bool NeedToCreateParamArray(ExpressionNode First, FunctionParameter Param)
    {
    ...
    public override PluginResult NewNode(ref ExpressionNode Node)
    {
        ...
        if (Node is StrExpressionNode) return PluginResult.Interrupt;

        if (Node is LinkingNode) return PluginResult.Interrupt;
