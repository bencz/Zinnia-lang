...
public class TypeMngrPlugin : ExpressionPlugin
{
    ...
    private bool IsProcessed(Identifier Type)
    {
    ...
    public ExpressionNode Convert(ExpressionNode Node, Identifier To, CodeString Code)
    {
        ...
        if (OpFuncRes != SimpleRecResult.Unknown)
        {
            ...
            if (OpFuncRes == SimpleRecResult.Failed) return null;
    ...
    private TupleType CreateTupleTypeForNodes(ExpressionNode Node)
    {
    ...
    private bool CheckTupleType(ExpressionNode Node)
    {
        ...
        for (var i = 0; i < Node.Children.Length; i++)
            if (Node.Children[i].Type != Members[i].Children[0])
                return false;

        return true;
    }

    private PluginResult CheckFunctionCall(ref ExpressionNode Node)
    {
    ...
    public PluginResult ProcessOperatorFunctions(ref ExpressionNode Node)
    {
        ...
        if (Op == Operator.Cast)
        {
            ...
            if (Res != SimpleRecResult.Unknown)
            {
                ...
                if (Res == SimpleRecResult.Failed) return PluginResult.Failed;
                throw new ApplicationException();
            }
    ...
    private bool CalculateIdentifiers(Identifier Type, ObjectInitNode Node)
    {
        var RetValue = true;
    ...
    private bool CheckArrayInitNode(Identifier Type, ArrayInitNode Node)
    {
        var RetValue = true;
    ...
    private bool NeedConvertToNonstaticFunction(ExpressionNode Node)
    {
        return Expressions.IsSelfSpecified(Node) && (Node.Flags & ExpressionFlags.FixedType) == 0;
    }

    private ExpressionNode ConvertToNonstaticFunction(ExpressionNode Node, CodeString Code)
    {
    ...
    private bool ConvertChildrenToNonstaticFunction(ExpressionNode Node)
    {
    ...
    public PluginResult NewOpNode(ref ExpressionNode Node)
    {
        ...
        else if (Op == Operator.Member)
        {
            ...
            if (IdNode != null && IdNode.Identifier is Constructor &&
                (Flags & TypeMngrPluginFlags.AllowConstructorCalls) == 0)
            {
        ...
        else if (Op == Operator.Index)
        {
            ...
            if (Type is StructuredType)
            {
                ...
                if (Indexer != null)
                {
                    ...
                    var NewCh = new[] { Ch[0], IndexerNode };
    ...
    private ExpressionNode ExtractMember(ExpressionNode Node, int Index, CodeString Code)
    {
        ...
        var Ch = new[] { Node, Member };
        return Parent.NewNode(new OpExpressionNode(Operator.Member, Ch, Code));
    }

    private bool CheckAssignVar(ExpressionNode Dst, CodeString Code)
    {
    ...
    private bool CheckAssignVar(ExpressionNode Dst, Identifier SrcType, CodeString Code)
    {
    ...
    private ExpressionNode ResolveAutomaticTypes(ExpressionNode Src, CodeString Code)
    {
        if (Src is OpExpressionNode) return ResolveAutomaticTypes_OpNode(Src, Code);

        return Src;
    }

    private ExpressionNode ResolveAutomaticTypes_OpNode(ExpressionNode Src, CodeString Code)
    {
    ...
    private ExpressionNode CheckAssignVar(ExpressionNode Dst, ExpressionNode Src, CodeString Code)
    {
        Dst = Dst.RealNode;
    ...
    public Identifier CastToAppropriateTypes(ref Identifier Type0, bool AutoConvert0, ref Identifier Type1,
        bool AutoConvert1)
    {
        var RealId0 = Type0.RealId;
        var RealId1 = Type1.RealId;

        if (RealId0 is AutomaticType)
        {
            Type0 = Type1;
            return Type0;
        }

        if (RealId1 is AutomaticType)
        {
            Type1 = Type0;
            return Type0;
        }

        if (RealId0 is TupleType)
        {
            Type1 = Type0;
            return Type0;
        }

        if (RealId1 is TupleType)
        {
            Type0 = Type1;
            return Type0;
        }
        ...
        if (RealId0 is NumberType && RealId1 is NumberType)
        {
            ...
            if (AutoConvert1 && (!(RealId1 is FloatType) || RealId0 is FloatType))
            {
                Type1 = Type0;
                return Type0;
            }
    ...
    private bool CheckVariablesTypes(List<Identifier> Identifiers, List<Identifier> Types)
    {
    ...
    private PluginResult CastToAppropriateTypes(ref ExpressionNode Node)
    {
        var Ch = Node.Children;
        var Type0 = Ch[0].Type;
        var Type1 = Ch[1].Type;
        ...
        if (Type0.RealId is TupleType && Type1.RealId is TupleType)
        {
            ...
            var Members0 = Tuple0 != null
                ? Tuple0.StructuredScope.IdentifierList
                : new AutoAllocatedList<Identifier>();

            var Members1 = Tuple1 != null
                ? Tuple1.StructuredScope.IdentifierList
                : new AutoAllocatedList<Identifier>();
    ...
    private Identifier GetNonrefType(Identifier Type)
    {
    ...
    private bool IsUnlinkableNode(ExpressionNode Node)
    {
        var Op = Expressions.GetOperator(Node);
        var Ch = Node.Children;
    ...
    private bool NeedToResolveAutomaticTypes(ExpressionNode Node)
    {
        var Op = Expressions.GetOperator(Node);
        var Ch = Node.Children;

        if (Op == Operator.NewObject) return Ch == null || Ch.Length == 0;
        if (Op == Operator.Assignment) return Ch[0].Type.RealId is AutomaticType;
        return Op != Operator.Cast && Op != Operator.Tuple && Op != Operator.Call;
    }

    private PluginResult ResolveAutomaticTypes_Tuple(ref ExpressionNode Node, CodeString Code)
    {
