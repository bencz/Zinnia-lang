...
{
    public static string[] Strings = { "try", "catch", "finally" };
...
{
    public static string[] Strings = { "switch", "case", "default" };
    ...
    public SimpleRecResult Recognize(CodeScopeNode Scope, CodeString Code)
    {
        ...
        var Result = ZinniaHelper.ForEachLine(SwitchScope, Line =>
        {
            ...
            if (LocalRes.Index == 1)
            {
                ...
                CompareNode.Children = new[] { Left, Right };
...
public class RepeatCommRecognizer : LanguageNode, ICommRecognizer
{
    ...
    public SimpleRecResult Recognize(CodeScopeNode Scope, CodeString Code)
    {
        if (Code.StartsWith(String, new IdCharCheck(true)))
        {
            ...
            if (IdNode != null)
            {
                var CondCh = new[] { IdNode, Condition };
...
public class ForInCommRecognizer : LanguageNode, ICommRecognizer
{
    ...
    public static string[] Strings = { "in" };
    public static string[] StepStrings = { "step" };
    ...
    public SimpleRecResult Recognize(CodeScopeNode Scope, CodeString Code)
    {
        if (Code.StartsWith(String, new IdCharCheck(true)))
        {
            ...
            for (var i = 0; i < RangeNodes.Length; i++)
            {
                ...
                //-------------------------------------------------------------------------
                var Variable = VarDecls[i].ToVariable(Command, true);
                if (Variable == null) return SimpleRecResult.Failed;
    ...
    private static ExpressionNode GetChild(ExpressionNode Node, int Index, CodeString Code)
    {
    ...
    private static int GetTupleLength(ExpressionNode Node, int Default = 1)
    {
        if (Expressions.GetOperator(Node) == Operator.Tuple)
    ...
    private static bool CheckLength(CompilerState State, ExpressionNode Node, ExpressionNode Step, int Count)
    {
    ...
    static RangeNodePair[] MapRangeNodes(CompilerState State, CodeString Code, ExpressionNode Node,
        ExpressionNode Step = null, int Count = -1)
    {
        if (Step == null)
        {
    ...
    private ConditionResult IsNegative(ExpressionNode Node)
    {
        ...
        if (CNode.Value is IntegerValue)
        {
        ...
        if (CNode.Value is ZeroValue) return ConditionResult.False;

        return ConditionResult.Unknown;
    }

    private List<ExpressionNode> GetExpressions(Command Command, Identifier Variable,
    ...
    private struct RangeNodePair
    {
        public readonly ExpressionNode Init;
        public readonly ExpressionNode Until;
...
{
    public static string String = "for";
    public static string[] Strings = { "to", "downto", "until", "eachin" };
    public static string[] StepString = { "step" };
    ...
    public SimpleRecResult Recognize(CodeScopeNode Scope, CodeString Code)
    {
        if (Code.StartsWith(String, new IdCharCheck(true)))
        {
            ...
            var ForVar = VarDecls[0].ToVariable(ForCommand, true);
            ...
            if (FindRes.Index < 3)
            {
                ...
                var StepStr = new CodeString();
                var StepRes = UntilToStr.Find(StepString, IdCharCheck: new IdCharCheck(true),
                    Handlers: SkippingHandlers);
                if (StepRes.Index != -1)
                {
                ...
                var CmpList = new[] { ForVarNode, UntilTo };
...
{
    public static string[] Strings = { "throw", "rethrow" };
...
public class WithCommRecognizer : LanguageNode, ICommRecognizer
{
    ...
    public SimpleRecResult Recognize(CodeScopeNode Scope, CodeString Code)
    {
        if (Code.StartsWith(String, new IdCharCheck(true)))
        {
            ...
            else
            {
                ...
                NewScope.WithNode = new IdExpressionNode(Var, NodeStr);
                if (Ref)
                {
    ...
    {
        if (Node is IdExpressionNode) return false;
...
public class CtorCallRecognizer : AfterDeclarationRecognizer
{
    ...
    private Constructor GetConstructor(CodeScopeNode Scope, Identifier Class, ExpressionNode[] Ch, CodeString Code)
    {
    ...
    private ExpressionNode GetConstructorNode(PluginRoot Plugin, Identifier Class, ExpressionNode[] Ch, CodeString Code)
    {
        ...
        var MemberCh = new[] { SelfNode, ConstructorNode };
        return Plugin.NewNode(new OpExpressionNode(Operator.Member, MemberCh, Code));
    }

    private ExpressionNode GetConstructorCaller(PluginRoot Plugin, Identifier Class, ExpressionNode[] Ch,
        CodeString Code)
    {
    ...
    public override SimpleRecResult Recognize(CodeScopeNode Scope, CodeString Code)
    {
            ...
            Code.StartsWith(CodeProcessor.BaseName, new IdCharCheck(true)))
        {
            ...
            else if (Function.IsEqual(CodeProcessor.BaseName))
            {
                ...
                if (Type.BaseStructures.Length > 1)
                {
    ...
    public SimpleRecResult Recognize(CodeScopeNode Scope, CodeString Code)
    {
        ...
        if (Code.Find(';', Handlers: SkippingHandlers) != -1)
        {
            ...
            for (var i = 0; i < Spl.Count; i++)
                if (!Scope.RecognizeCommand(Spl[i]))
                    RetValue = false;
